...
function answer(t, n) {
  ...
  for(let i = 0; i < t; i++) {
    let nextBoard = new Array(n + 1).fill(0);
    nextBoard[n] = board[n];

    for(let tile = Math.max(1, n - t + i); tile < n; tile++) {
      nextBoard[tile - 1] += board[tile] % 123454321
      nextBoard[tile] += board[tile] % 123454321
      nextBoard[tile + 1] += board[tile] % 123454321
    }
    board = nextBoard;
  }
  return board[n] % 123454321
}
